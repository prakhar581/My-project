...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        """NOTE:listofPossiblePlates(list of object of PossiblePlates ) is possiblePlate"""
        ...
        if Main.showSteps == True: # show steps ###################################################
            ...
            for possibleChar in listOfPossibleCharsInPlate: # Like M then C etc etc in MCLRNF1
                # To call PossibleChar(class) possibleChar=PosibleChar(contour_value
                # now accessing is done using possibleChar.contour(it's just an attribute) 
        ...
        if Main.showSteps == True: # show steps ###################################################
            ...
            for listOfMatchingChars in listOfListsOfMatchingCharsInPlate:
                ...
                cv2.drawContours(imgContours, contours, -1, (intRandomBlue, intRandomGreen, intRandomRed))
            # end for
            cv2.imshow("8", imgContours) # for 1 extracted plate 
        # end if # show steps #####################################################################
        ...
        for i in range(0, len(listOfListsOfMatchingCharsInPlate)):
            if len(listOfListsOfMatchingCharsInPlate[i]) > intLenOfLongestListOfChars:
                ...
                intIndexOfLongestListOfChars = i # i=0(our plate is at 1st loc)
            # end if
        # end for
        ...
        if Main.showSteps == True: # show steps ###################################################
            ...
            cv2.imshow("9", imgContours) # see pdf 2nd last step
        # end if # show steps #####################################################################
    ...
    return listOfPossiblePlates # takes listOfPossiblePlates and return same list
# end function
...
# possibleChar stores width, height, centre_x ,centre_y, Diagonal and aspect ration and ue it latter       
        possibleChar = PossibleChar.PossibleChar(contour)

        if checkIfPossibleChar(possibleChar): # Rough check for a segment of original image   # if contour is a possible char,       note this does not compare to other chars (yet) . . .
...
#recursiveListOfListsOfMatchingChars double check that we don't remove possible char
...
#listOfMatchingChars like MCLRNF1
def removeInnerOverlappingChars(listOfMatchingChars): #listOfMatchingChars bcz its a subset of listoflistofMatchingCharsinPlates[i]
    listOfMatchingCharsWithInnerCharRemoved = list(listOfMatchingChars)                # this will be the return value
   # These three lines are like permutation 
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars): #listOfMatchingChars is longestListofMatchingChars(MCLRNF1)
    strChars = ""               # this will be the return value, the chars in the lic plate

    height, width = imgThresh.shape #is cropped listofPossiblePlates

    imgThreshColor = np.zeros((height, width, 3), np.uint8)

    listOfMatchingChars.sort(key = lambda matchingChar: matchingChar.intCenterX)        # sort chars from left to right

    imgThreshColor=cv2.cvtColor(imgThresh, cv2.COLOR_GRAY2BGR)                     # make color version of threshold image so we can draw contours in color on it

    for currentChar in listOfMatchingChars:                                         # for each char in plate
        pt1 = (currentChar.intBoundingRectX, currentChar.intBoundingRectY)
        pt2 = ((currentChar.intBoundingRectX + currentChar.intBoundingRectWidth), (currentChar.intBoundingRectY + currentChar.intBoundingRectHeight))

        cv2.rectangle(imgThreshColor, pt1, pt2, Main.SCALAR_GREEN, 2)#second last step           # draw green box around the char
